...
struct GPU::Impl {
    ...
    void ReleaseChannel(Control::ChannelState& to_release) { UNIMPLEMENTED(); }
    ...
    void FlushCommands() { rasterizer->FlushCommands(); }
    ...
    template<typename Func> [[nodiscard]] u64 RequestSyncOperation(Func&& action)
    {
    ...
    [[nodiscard]] VideoCore::RendererBase& Renderer() { return *renderer; }

    /// Returns a const reference to the underlying renderer.
    [[nodiscard]] const VideoCore::RendererBase& Renderer() const { return *renderer; }

    /// Returns a reference to the shader notifier.
    [[nodiscard]] VideoCore::ShaderNotify& ShaderNotify() { return *shader_notify; }

    /// Returns a const reference to the shader notifier.
    [[nodiscard]] const VideoCore::ShaderNotify& ShaderNotify() const { return *shader_notify; }
    ...
    [[nodiscard]] bool IsAsync() const { return is_async; }

    [[nodiscard]] bool UseNvdec() const { return use_nvdec; }

    void RendererFrameEndNotify() { system.GetPerfStats().EndGameFrame(); }
    ...
    void ReleaseContext() { cpu_context->DoneCurrent(); }
    ...
    void FlushRegion(DAddr addr, u64 size) { gpu_thread.FlushRegion(addr, size); }
    ...
    void InvalidateRegion(DAddr addr, u64 size) { gpu_thread.InvalidateRegion(addr, size); }

    bool OnCPUWrite(DAddr addr, u64 size) { return rasterizer->OnCPUWrite(addr, size); }
