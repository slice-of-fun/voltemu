...
template<typename T> [[nodiscard]] T Arg(const IR::Value& value)
{
...
template<typename T, typename ImmFn> bool FoldCommutative(IR::Inst& inst, ImmFn&& imm_fn)
{
...
template<typename Func> bool FoldWhenAllImmediates(IR::Inst& inst, Func&& func)
{
...
template<typename Range> bool AreEqual(const Range& range)
{
...
template<typename T> void FoldAdd(IR::Block& block, IR::Inst& inst)
{
