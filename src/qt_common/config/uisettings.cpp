...
const std::array<Shortcut, 33> default_hotkeys{{
    ...
    {QStringLiteral(QT_TRANSLATE_NOOP("Hotkeys", "Exit Volt")).toStdString(),                  QStringLiteral(QT_TRANSLATE_NOOP("Hotkeys", "Main Window")).toStdString(), {std::string("Ctrl+Q"),  std::string("Home+Minus"), Qt::WindowShortcut, false}},
...
void RestoreWindowState(std::unique_ptr<QtConfig>& qtConfig)
{
    ...
    if (!FS::Exists(window_state_config_loc) && qtConfig->Exists("UI", "UILayout\\geometry")) {
        const auto config_loc =
            FS::PathToUTF8String(FS::GetVoltPath(FS::VoltPath::ConfigDir) / "qt-config.ini");
