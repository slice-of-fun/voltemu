...
public:
    ...
    [[nodiscard]] constexpr RBEntry& GetRBEntry() { return m_entry; }
    [[nodiscard]] constexpr const RBEntry& GetRBEntry() const { return m_entry; }

    constexpr void SetRBEntry(const RBEntry& entry) { m_entry = entry; }
};
static_assert(sizeof(IntrusiveRedBlackTreeNode) ==
              3 * sizeof(void*) + std::max<size_t>(sizeof(freebsd::RBColor), 4));
#pragma pack(pop)

template<class T, class Traits, class Comparator> class IntrusiveRedBlackTree;
...
private:
    template<class, class, class> friend class ::Common::IntrusiveRedBlackTree;
...
private:
    constexpr bool EmptyImpl() const { return m_root.IsEmpty(); }
...
public:
    ...
    static constexpr IntrusiveRedBlackTreeNode const* GetNext(IntrusiveRedBlackTreeNode const* node)
    {
        return static_cast<const IntrusiveRedBlackTreeNode*>(
            GetNext(const_cast<IntrusiveRedBlackTreeNode*>(node)));
    }

    static constexpr IntrusiveRedBlackTreeNode const* GetPrev(IntrusiveRedBlackTreeNode const* node)
    {
        return static_cast<const IntrusiveRedBlackTreeNode*>(
...
public:
    ...
    // Iterator accessors.
    constexpr iterator begin() { return iterator(this->GetMinImpl()); }

    constexpr const_iterator begin() const { return const_iterator(this->GetMinImpl()); }

    constexpr iterator end() { return iterator(static_cast<IntrusiveRedBlackTreeNode*>(nullptr)); }
    ...
    constexpr iterator iterator_to(reference ref) { return iterator(std::addressof(ref)); }
    ...
    // Content management.
    constexpr bool empty() const { return this->EmptyImpl(); }

    constexpr reference back() { return *this->GetMaxImpl(); }

    constexpr const_reference back() const { return *this->GetMaxImpl(); }

    constexpr reference front() { return *this->GetMinImpl(); }

    constexpr const_reference front() const { return *this->GetMinImpl(); }
...
template<typename T, typename Default> consteval auto* GetRedBlackKeyType()
{
...
public:
    ...
    private:
        ...
        constexpr ImplIterator GetImplIterator() const { return m_impl; }

    public:
        constexpr bool operator==(const Iterator& rhs) const { return m_impl == rhs.m_impl; }

        constexpr pointer operator->() const { return Traits::GetParent(std::addressof(*m_impl)); }
        ...
        constexpr operator Iterator<true>() const { return Iterator<true>(m_impl); }
    };
...
private:
    ...
    FindExistingImpl(IntrusiveRedBlackTreeNode const* node) const
    {
...
public:
    ...
    constexpr reference back() { return *Traits::GetParent(std::addressof(m_impl.back())); }
    ...
    constexpr reference front() { return *Traits::GetParent(std::addressof(m_impl.front())); }
    ...
    constexpr iterator erase(iterator it) { return iterator(m_impl.erase(it.GetImplIterator())); }
...
private:
    template<class, class, class> friend class IntrusiveRedBlackTree;
...
template<class Derived> class IntrusiveRedBlackTreeBaseTraits {
public:
    template<class Comparator>
