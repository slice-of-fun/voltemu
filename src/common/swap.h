...
template<typename T, typename F> struct SwapStructT {
...
public:
    ...
    template<typename S> SwappedT& operator=(const S& source)
    {
        value = swap(T(source));
        return *this;
    }
    ...
    // +v
    SwappedT operator+() const { return +swap(); }
    // -v
    SwappedT operator-() const { return -swap(); }

    // v / 5
    SwappedT operator/(const SwappedT& i) const { return swap() / i.swap(); }
    template<typename S> SwappedT operator/(const S& i) const { return swap() / i; }
    ...
    // v + 5
    SwappedT operator+(const SwappedT& i) const { return swap() + i.swap(); }
    template<typename S> SwappedT operator+(const S& i) const { return swap() + T(i); }
    // v - 5
    SwappedT operator-(const SwappedT& i) const { return swap() - i.swap(); }
    template<typename S> SwappedT operator-(const S& i) const { return swap() - T(i); }
    ...
    template<typename S> SwappedT& operator+=(const S& i)
    {
        value = swap(swap() + T(i));
        return *this;
    }
    ...
    // Comparison
    // v == i
    bool operator==(const SwappedT& i) const { return swap() == i.swap(); }
    ...
    SwappedT operator&(const SwappedT& b) const { return swap() & b.swap(); }
    template<typename S> SwappedT operator&(const S& b) const { return swap() & b; }
    SwappedT& operator&=(const SwappedT& b)
    {
        value = swap(swap() & b.swap());
        return *this;
    }
    template<typename S> SwappedT& operator&=(const S b)
    {
        value = swap(swap() & b);
        return *this;
    }
    ...
    template<typename S> SwappedT operator<<(const S& b) const { return swap() << b; }
    template<typename S> SwappedT& operator<<=(const S& b) const
    {
        value = swap(swap() << b);
        return *this;
    }
    ...
    // Arithmetic
    template<typename S, typename T2, typename F2> friend S operator+(const S& p, const SwappedT v);
...
// Arithmetic
template<typename S, typename T, typename F> S operator+(const S& i, const SwapStructT<T, F> v)
{
    return i + v.swap();
}
...
// Arithmetic + assignments
template<typename S, typename T, typename F> S& operator+=(S& i, const SwapStructT<T, F> v)
{
    i += v.swap();
    return i;
}
...
// Comparison
template<typename S, typename T, typename F> bool operator<(const S& p, const SwapStructT<T, F> v)
{
    return p < v.swap();
}
template<typename S, typename T, typename F> bool operator>(const S& p, const SwapStructT<T, F> v)
...
template<typename T> struct Swap64T {
    static T swap(T x) { return T(Common::swap64(x)); }
};

template<typename T> struct Swap32T {
    static T swap(T x) { return T(Common::swap32(x)); }
};

template<typename T> struct Swap16T {
    static T swap(T x) { return T(Common::swap16(x)); }
};

template<typename T> struct SwapFloatT {
    static T swap(T x) { return T(Common::swapf(x)); }
};

template<typename T> struct SwapDoubleT {
    static T swap(T x) { return T(Common::swapd(x)); }
};

template<typename T> struct SwapEnumT {
...
public:
    ...
    explicit operator base() const { return base(swap(value)); }
                ...
                std::conditional_t<std::is_same_v<base, s32>, Swap32T<s32>,
                                   std::conditional_t<std::is_same_v<base, u64>, Swap64T<u64>,
                                                      std::conditional_t<std::is_same_v<base, s64>,
                                                                         Swap64T<s64>, void>>>>>>;
    static T swap(T x) { return T(swap_t::swap(base(x))); }
};
