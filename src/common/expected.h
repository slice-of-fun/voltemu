...
template<typename E> constexpr auto operator<=>(const Unexpected<E>& lhs, const Unexpected<E>& rhs)
{
...
template<typename T, typename E> struct expected_operations_base : expected_storage_base<T, E> {
    ...
    template<typename Rhs> void construct_with(Rhs&& rhs) noexcept
    {
    ...
    template<typename Rhs> void assign_common(Rhs&& rhs)
    {
        if (this->m_has_val) {
            if (rhs.m_has_val) {
...
struct expected_copy_base<T, E, false> : expected_operations_base<T, E> {
    ...
    expected_copy_base(const expected_copy_base& rhs) : expected_operations_base<T, E>{no_init_t{}}
    {
        if (rhs.has_value()) {
...
public:
    ...
    constexpr explicit Expected(Expected<U, G>&& rhs) : ctor_base{detail::default_constructor_tag{}}
    {
        if (rhs.has_value()) {
    ...
    template<typename U = T> constexpr U& operator*() & { return val(); }

    template<typename U = T> constexpr const U& operator*() const& { return val(); }

    template<typename U = T> constexpr U&& operator*() && { return std::move(val()); }

    template<typename U = T> constexpr const U&& operator*() const&& { return std::move(val()); }

    constexpr bool has_value() const noexcept { return this->m_has_val; }

    constexpr explicit operator bool() const noexcept { return this->m_has_val; }

    template<typename U = T> constexpr U& value() & { return val(); }

    template<typename U = T> constexpr const U& value() const& { return val(); }

    template<typename U = T> constexpr U&& value() && { return std::move(val()); }

    template<typename U = T> constexpr const U&& value() const&& { return std::move(val()); }
...
private:
    ...
    template<typename U = T> constexpr U& val() { return this->m_val; }

    template<typename U = T> constexpr const U& val() const { return this->m_val; }
