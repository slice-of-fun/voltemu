      ...
      ui(std::make_unique<Ui::DirectConnect>()), system{system_}
{
    ...
    if (ui->nickname->text().isEmpty() && !Settings::values.volt_username.GetValue().empty()) {
        // Use yuzu Web Service user name as nickname by default
        ui->nickname->setText(QString::fromStdString(Settings::values.volt_username.GetValue()));
    }
