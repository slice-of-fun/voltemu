...
template<typename T> class KScopedAutoObject;
class KThread;
} // namespace Kernel
...
public:
    ...
    bool IsDomain() const { return is_domain; }
    ...
    void ConvertToDomainOnRequestEnd() { convert_to_domain = true; }

    std::size_t DomainHandlerCount() const { return domain_handlers.size(); }

    bool HasSessionHandler() const { return session_handler != nullptr; }

    SessionRequestHandler& SessionHandler() { return *session_handler; }

    const SessionRequestHandler& SessionHandler() const { return *session_handler; }
    ...
    Service::ServerManager& GetServerManager() { return server_manager; }
    ...
    bool GetIsInitializedForSm() const { return is_initialized_for_sm; }

    void SetIsInitializedForSm() { is_initialized_for_sm = true; }
...
public:
    ...
    [[nodiscard]] u32* CommandBuffer() { return cmd_buf.data(); }

    /**
     * Returns the session through which this request was made. This can be used as a map key to
     * access per-client data on services.
     */
    [[nodiscard]] Kernel::KServerSession* Session() { return server_session; }
    ...
    [[nodiscard]] u32_le GetHipcCommand() const { return command; }
    ...
    [[nodiscard]] bool IsTipc() const { return command_header->IsTipc(); }

    [[nodiscard]] IPC::CommandType GetCommandType() const { return command_header->type; }

    [[nodiscard]] u64 GetPID() const { return pid; }

    [[nodiscard]] u32 GetDataPayloadOffset() const { return data_payload_offset; }
    ...
    [[nodiscard]] bool HasDomainMessageHeader() const { return domain_message_header.has_value(); }
    ...
    void AddMoveObject(Kernel::KAutoObject* object) { outgoing_move_objects.emplace_back(object); }

    void AddMoveInterface(SessionRequestHandlerPtr s);

    void AddCopyObject(Kernel::KAutoObject* object) { outgoing_copy_objects.emplace_back(object); }
    ...
    template<typename T> std::shared_ptr<T> GetDomainHandler(std::size_t index) const
    {
    ...
    [[nodiscard]] Kernel::KThread& GetThread() { return *thread; }

    [[nodiscard]] Core::Memory::Memory& GetMemory() const { return memory; }

    template<typename T> Kernel::KScopedAutoObject<T> GetObjectFromHandle(u32 handle)
    {
    ...
    bool GetIsDeferred() const { return is_deferred; }

    void SetIsDeferred(bool is_deferred_ = true) { is_deferred = is_deferred_; }
