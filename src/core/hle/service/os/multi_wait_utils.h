...
public:
    ...
    ~AutoMultiWaitHolder() { m_holder.UnlinkFromMultiWait(); }
...
inline std::pair<MultiWaitHolder*, int>
WaitAnyImpl(Kernel::KernelCore& kernel, MultiWait* multi_wait, WaitAnyFunction func, int)
{
...
inline std::pair<MultiWaitHolder*, int>
WaitAnyImpl(Kernel::KernelCore& kernel, MultiWait* multi_wait, WaitAnyFunction func, Args&&... args)
{
...
requires(sizeof...(Args) >
         0) inline std::pair<MultiWaitHolder*, int> WaitAny(Kernel::KernelCore& kernel,
...
template<typename... Args>
requires(sizeof...(Args) > 0) inline int WaitAny(Kernel::KernelCore& kernel, Args&&... args)
{
    return impl::WaitAnyImpl(kernel, &MultiWait::WaitAny, std::forward<Args>(args)...).second;
}

template<typename... Args>
requires(sizeof...(Args) > 0) inline std::pair<MultiWaitHolder*, int> TryWaitAny(
    Kernel::KernelCore& kernel, MultiWait* multi_wait, Args&&... args)
{
...
template<typename... Args>
requires(sizeof...(Args) > 0) inline impl::NotBoolButInt
    TryWaitAny(Kernel::KernelCore& kernel, Args&&... args)
{
