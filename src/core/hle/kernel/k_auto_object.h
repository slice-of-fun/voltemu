...
protected:
    ...
    public:
        ...
        constexpr ClassTokenType GetClassToken() const { return m_class_token; }
...
public:
    ...
    u32 GetReferenceCount() const { return m_ref_count.load(); }
    ...
    template<typename Derived> Derived DynamicCast()
    {
    ...
    template<typename Derived> const Derived DynamicCast() const
    {
        static_assert(std::is_pointer_v<Derived>);
        using DerivedType = std::remove_pointer_t<Derived>;
...
public:
    virtual u64 GetId() const { return reinterpret_cast<u64>(this); }
...
public:
    ...
    constexpr T* operator->() { return m_obj; }
    constexpr T& operator*() { return *m_obj; }

    constexpr void Reset(T* o) { KScopedAutoObject(o).Swap(*this); }

    constexpr T* GetPointerUnsafe() { return m_obj; }

    constexpr T* GetPointerUnsafe() const { return m_obj; }
    ...
    constexpr bool IsNull() const { return m_obj == nullptr; }
    constexpr bool IsNotNull() const { return m_obj != nullptr; }

private:
    template<typename U> friend class KScopedAutoObject;

private:
    T* m_obj{};

private:
    constexpr void Swap(KScopedAutoObject& rhs) noexcept { std::swap(m_obj, rhs.m_obj); }
};
