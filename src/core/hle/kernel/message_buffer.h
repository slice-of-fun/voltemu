...
public:
    ...
    public:
        ...
        constexpr u16 GetTag() const { return m_header.tag; }

        constexpr s32 GetPointerCount() const { return m_header.pointer_count; }

        constexpr s32 GetSendCount() const { return m_header.send_count; }

        constexpr s32 GetReceiveCount() const { return m_header.receive_count; }

        constexpr s32 GetExchangeCount() const { return m_header.exchange_count; }
        ...
        constexpr s32 GetRawCount() const { return m_header.raw_count; }
        ...
        constexpr s32 GetReceiveListOffset() const { return m_header.receive_list_offset; }
        ...
        constexpr const u32* GetData() const { return m_header.raw.data(); }

        static constexpr size_t GetDataSize() { return sizeof(m_header); }
    };
    ...
    public:
        ...
        constexpr bool GetHasProcessId() const { return m_header.has_process_id.Value() != 0; }

        constexpr s32 GetCopyHandleCount() const { return m_header.copy_handle_count; }

        constexpr s32 GetMoveHandleCount() const { return m_header.move_handle_count; }

        constexpr const u32* GetHeader() const { return m_header.raw.data(); }
    ...
    public:
        ...
        constexpr Attribute GetAttribute() const { return m_data.attributes; }

        constexpr const u32* GetData() const { return m_data.raw.data(); }

        static constexpr size_t GetDataSize() { return sizeof(m_data); }
    };
    ...
    public:
        ...
        constexpr s32 GetIndex() const { return m_data.index; }
        ...
        constexpr size_t GetSize() const { return m_data.size; }
...
public:
    ...
    constexpr void* GetBufferForDebug() const { return m_buffer; }

    constexpr size_t GetBufferSize() const { return m_size; }
    ...
    template<typename T> const T& GetRaw(s32 index) const
    {
        return *reinterpret_cast<const T*>(m_buffer + index);
    }

    template<typename T> s32 SetRaw(s32 index, const T& val) const
    {
        *reinterpret_cast<const T*>(m_buffer + index) = val;
        return index + (Common::AlignUp(sizeof(val), sizeof(*m_buffer)) / sizeof(*m_buffer));
    }

    void GetRawArray(s32 index, void* dst, size_t len) const { memcpy(dst, m_buffer + index, len); }
    ...
    void SetNull() const { this->Set(MessageHeader()); }
    ...
    u32 Get32(s32 index) const { return m_buffer[index]; }
    ...
    u64 GetProcessId(s32 index) const { return this->Get64(index); }
    ...
    static constexpr size_t GetMessageBufferSize(const MessageHeader& hdr, const SpecialHeader& spc)
    {
