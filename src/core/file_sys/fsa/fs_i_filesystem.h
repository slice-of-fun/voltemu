...
public:
    ...
    Result DeleteFile(const Path& path) { R_RETURN(this->DoDeleteFile(path)); }

    Result CreateDirectory(const Path& path) { R_RETURN(this->DoCreateDirectory(path)); }

    Result DeleteDirectory(const Path& path) { R_RETURN(this->DoDeleteDirectory(path)); }
    ...
    Result Commit() { R_RETURN(this->DoCommit()); }
    ...
    Result CommitProvisionally(s64 counter) { R_RETURN(this->DoCommitProvisionally(counter)); }

    Result Rollback() { R_RETURN(this->DoRollback()); }
...
private:
    ...
    Result DoDeleteFile(const Path& path) { R_RETURN(backend.DeleteFile(path.GetString())); }
    ...
    Result DoCommit() { R_THROW(ResultNotImplemented); }

    Result DoGetFreeSpaceSize(s64* out, const Path& path) { R_THROW(ResultNotImplemented); }

    Result DoGetTotalSpaceSize(s64* out, const Path& path) { R_THROW(ResultNotImplemented); }
    ...
    // These aren't accessible as commands
    Result DoCommitProvisionally(s64 counter) { R_THROW(ResultNotImplemented); }

    Result DoRollback() { R_THROW(ResultNotImplemented); }

    Result DoFlush() { R_THROW(ResultNotImplemented); }
