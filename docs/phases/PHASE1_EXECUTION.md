...
| # | Task | Status | Notes |
|---|------|--------|-------|
| 1.1 | Run `scripts/rebranding/02_audit.sh` on clean clone | ?? | Baseline established by direct inspection (script not run); see legal audit |
...
| 2.3 | Rename CMake targets: `volt-*` ? `volt-*` | ?? | Main target is `volt` (`src/yuzu/CMakeLists.txt:17,244`); **leftover `volt` target at `:366`** in APPLE+Xcode block — latent configure error, see rebrand audit |
