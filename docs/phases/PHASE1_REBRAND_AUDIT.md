...
A **no-build inspection** of remaining "Volt" references for the Volt rebrand,
...
| `VOLT_` CMake variables removed | ✅ done | 0 `VOLT_` vars in non-vendored `*.cmake`/`CMakeLists.txt` |
...
- **`src/yuzu/CMakeLists.txt:366`** — `set_target_properties(volt PROPERTIES ...)`
  references a target named `volt` that **no longer exists** (renamed to `volt`).
  ...
  a **configure-time error**. Adjacent stale asset refs: `:364` `volt.icon`,
  `:374` `volt.icns`.
  ...
  > (`dist/volt.icns`, `dist/volt.icon`) are part of the 1.6 asset rework.

### `src/` source leftover
- **`VOLT_DIR "volt"`** (5 refs in `fs_paths.h` + consumers) — the on-disk user
...
52 user-facing "Volt" references found across desktop (Qt), macOS, and Android.
...
| Ref | File:line | Risk |
|-----|-----------|------|
| `setOrganizationName("volt")` | `src/yuzu/main.cpp:115` | Relocates **all** stored config (Windows registry / macOS prefs / Linux `~/.config`) |
| `setApplicationName("volt")` | `src/yuzu/main.cpp:116` | Same — config/data path key |
| `setDesktopFileName("dev.volt_emu.volt")` | `src/yuzu/main.cpp:158` | Must match the installed `.desktop` id (`dist/dev.volt_emu.volt.desktop`) — **currently mismatched** |
| `VOLT_DIR "volt"` | `src/common/fs/fs_paths.h` (+consumers) | On-disk data dir (see 1.3) |
| UTType ids `com.volt-emu.{xci,nsp,nca,nro,nso,xaml}` | `src/yuzu/Info.plist` (6) | macOS file-type associations; changing breaks existing "open with" links |

> Note: `setDesktopFileName` (`dev.volt_emu.volt`) already disagrees with the
...
**Qt `.ui` (desktop):**
- `aboutdialog.ui:14` "About Volt"; `:72` 28pt "Volt" heading; `:105` description; `:147` Nintendo disclaimer
- `configuration/configure.ui:20` "Volt Configuration"
- `configuration/configure_web.ui:25` "Volt Web Service"
- `deps_dialog.ui:14/:72/:85` "Volt Dependencies" + body; widget `labelVolt`
- `main.ui:62` "Open &Volt Folders"; `:294` "&About Volt"; `:573` "&Volt Dependencies"
- `ryujinx_dialog.ui:26/:28/:40` migration copy + "From Volt" button
- `configuration/system/new_user_dialog.ui:60` default "Volt"

**Qt `tr()` message-box titles (`tr("Volt")`):**
- `configure_general.cpp:108`; `configure_motion_touch.cpp:175,179,183,189,195,296` (6); `main_window.cpp:4589,4687,4700` (3)

**Other code strings:**
- `main_window.cpp:1937` `LOG_INFO(Frontend, "Volt starting...")`
- `main_window.cpp:1881` video-core error text "Volt has encountered…"
- `new_user_dialog.cpp:68` default username `"Volt"`
- `multiplayer/lobby.cpp:71,78` default nickname `"Volt"` *(moderate — also a stored value)*

**Generated artifact filenames:**
- `video_core/gpu_logging/gpu_logging.cpp:61,64` `volt_gpu.log` / `volt_gpu.log.old.txt`

### Tier C — Internal identifiers (optional; not user-visible)
Cosmetic only; may appear in crash/debug symbols. Lowest priority.
- `main_window.h:396` `OnVoltDependencies()`; `:590` `GetVoltCommand()`
- `main_window.cpp:1654` `ui->action_Volt_Dependencies`
...
| Area | Disposition |
|------|-------------|
| 1.3 CMake project/vars/target | ✅ mostly done; **1 latent macOS/Xcode bug** (`CMakeLists.txt:366`) + `VOLT_DIR` |
...
a build is available, are: (1) the `CMakeLists.txt:366` `volt`→`volt` target fix
