...
Use this guide to get starting using the Volt emulator.

**Click [Here](https://evilperson1337.notion.site/Volt-Quick-Start-2b057c2edaf6817b9859d8bcdb474017) for a version of this guide with images & visual elements.**
...
- The [*latest C++ Redistributable*](https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist?view=msvc-170#latest-supported-redistributable-version) from Microsoft.
    - Volt will not even launch without it see [*Volt Fails to Launch*](./Troubleshoot.md) for more information.
...
1. Download either the *Stable* or *Nightly* Volt application.
    <aside>
...
2. Extract the contents to wherever you want to store the program on your computer. 
    ...
    ***TIP***: If you want to run Volt completely portable (everything is stored in the folder you extracted it to) - create a folder called **user** if it is not there by default.
    
    </aside> 
3. Run ***Volt.exe*** to launch the program.  
    
    <aside>

    ***INFO***: You may get a “*Windows protected your PC”* SmartScreen message that appears.  This is just Windows Defender saying it did not recognize the application and did not run it - Volt is completely safe.  Click **More info** and then **Run anyway** to dismiss this message.
    
    </aside>
4. Volt will now launch and notify you about missing Decryption keys. Close the dialog box by hitting **OK**.
...
2. Download either the *Stable* or *Nightly* Volt AppImage onto your Steam Deck and save it somewhere accessible.
...
3. Double-Click the Volt executable to launch the program.
    <aside>
...
5. Volt will now launch and notify you about missing Encryption keys. Close the dialog box by hitting **OK**.
