...
## Volt Fails to Launch and Does Not Leave Any Logs

**Click [Here](https://evilperson1337.notion.site/Windows-Volt-Fails-to-Launch-and-Does-Not-Leave-Any-Logs-2b057c2edaf68156b640cf1ac549870a) for a version of this guide with images & visual elements.**
...
```
Faulting application name: volt.exe, version: 0.0.0.0, time stamp: 0x6795dc3c
...
Faulting application path: C:\temp\Volt-Windows\volt.exe
...
*Volt requires the latest C++ redistributable from Microsoft in order to run.  Like many other programs, it relies on aspects and libraries included in this runtime, without it - the program cannot run.*
...
4. Launch Volt again, if the main window appears, you are good to go.  If not, proceed to the next issue.
</aside>
...
*A corruption of necessary system files can cause odd behaviors when Volt tries to access them.  It is a very rare case and you would likely see other programs misbehaving if this is what your issue is, but you can try if you have no other options.*

1. Launch Volt to generate a crash.
...
9. Launch Volt and verify it is now working.
</aside>
