...
2. Open ReShade and hit browse, then the folder where `volt.exe` is at, hit open, then hit next.
...
Run the following batch script *inside* the Volt folder:
```cmd
@echo off
pushd "%~dp0"
